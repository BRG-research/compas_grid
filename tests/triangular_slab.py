...
quarter_polygon_offset : Polygon = Polygon([
    [board_thickness*0.5, -board_thickness*0.5, 0],
    [quarter_width-board_thickness*1.0, -board_thickness*0.5, 0],
    [quarter_width-board_thickness*1.0, quarter_depth-board_thickness, 0],
    [board_thickness*0.5, quarter_depth-board_thickness, 0],    
])
...
for i in range(quarter_divisions):
    
    # Linear interpolation of dicisions
    t : float = i / quarter_divisions

    # Rotate the line and its polygon 
    rotation : Rotation = Rotation.from_axis_and_angle([0,0,1], angle * t, point=quarter_polygon[0])
